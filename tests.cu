...
void compare_kernel_against_handwritten_example_1(int kernel) {
  ...
  case 1: {
    ...
    int8_t *deviceFilter;
    int nPixel = output_img.width*output_img.height;
    cudaMalloc((void**)&deviceIn,nPixel*sizeof(int32_t));
    cudaMalloc((void**)&deviceOut,nPixel*sizeof(int32_t));
    cudaMalloc((void**)&deviceFilter,FILTER_DIMENSION*FILTER_DIMENSION*sizeof(int8_t));

    cudaMemcpy(deviceIn,source.matrix,nPixel*sizeof(int32_t), cudaMemcpyHostToDevice);
    cudaMemcpy(deviceOut,source.matrix,nPixel*sizeof(int32_t), cudaMemcpyHostToDevice);
    cudaMemcpy(deviceFilter,FILTER,9*9*sizeof(int8_t),cudaMemcpyHostToDevice);
    run_kernel1(deviceFilter, FILTER_DIMENSION, deviceIn, deviceOut, output_img.width, output_img.height);
    cudaMemcpy(output_img.matrix,deviceOut,nPixel*sizeof(int32_t), cudaMemcpyDeviceToHost);
    cudaFree(deviceIn);
    cudaFree(deviceOut);
  ...
  case 2: {
    ...
    cudaMemcpy(deviceOut,source.matrix,nPixel*sizeof(int32_t), cudaMemcpyHostToDevice);
    cudaMemcpy(deviceFilter,FILTER,9*9*sizeof(int8_t),cudaMemcpyHostToDevice);
    run_kernel2(deviceFilter, FILTER_DIMENSION, deviceIn, deviceOut, output_img.width, output_img.height);
    cudaMemcpy(output_img.matrix,deviceOut,nPixel*sizeof(int32_t), cudaMemcpyDeviceToHost);
    cudaFree(deviceIn);
    cudaFree(deviceOut);
    cudaFree(deviceFilter);
  ...
  case 3: {
    ...
    cudaMemcpy(deviceOut,source.matrix,nPixel*sizeof(int32_t), cudaMemcpyHostToDevice);
    cudaMemcpy(deviceFilter,FILTER,9*9*sizeof(int8_t),cudaMemcpyHostToDevice);
    run_kernel3(deviceFilter, FILTER_DIMENSION, deviceIn, deviceOut, output_img.width, output_img.height);
    cudaMemcpy(output_img.matrix,deviceOut,nPixel*sizeof(int32_t), cudaMemcpyDeviceToHost);
    cudaFree(deviceIn);
    cudaFree(deviceOut);
    cudaFree(deviceFilter);
  ...
  case 4: {
    ...
    cudaMemcpy(deviceOut,source.matrix,nPixel*sizeof(int32_t), cudaMemcpyHostToDevice);
    cudaMemcpy(deviceFilter,FILTER,9*9*sizeof(int8_t),cudaMemcpyHostToDevice);
    run_kernel4(deviceFilter, FILTER_DIMENSION, deviceIn, deviceOut, output_img.width, output_img.height);
    cudaMemcpy(output_img.matrix,deviceOut,nPixel*sizeof(int32_t), cudaMemcpyDeviceToHost);
    cudaFree(deviceIn);
    cudaFree(deviceOut);
    cudaFree(deviceFilter);
  ...
  case 5: {
    ...
    cudaMemcpy(deviceOut,source.matrix,nPixel*sizeof(int32_t), cudaMemcpyHostToDevice);
    cudaMemcpy(deviceFilter,FILTER,9*9*sizeof(int8_t),cudaMemcpyHostToDevice);
    run_kernel5(FILTER_DIMENSION, deviceIn, deviceOut, output_img.width, output_img.height);
    cudaMemcpy(output_img.matrix,deviceOut,nPixel*sizeof(int32_t), cudaMemcpyDeviceToHost);
    cudaFree(deviceIn);
    cudaFree(deviceOut);
    cudaFree(deviceFilter);
...
TEST(csc367a4, cpu_handwritten_tests) {
...
TEST(csc367a4, compare_kernels_against_each_other) {
  ...
  for (int width = 1; width < WIDTH_MAX; ++width) {
    for (int height = 1; height < HEIGHT_MAX; ++height) {
      ...
      cudaMemcpy(deviceOut,source.matrix,nPixel*sizeof(int32_t), cudaMemcpyHostToDevice);
      cudaMemcpy(deviceFilter,FILTER,9*9*sizeof(int8_t),cudaMemcpyHostToDevice);
      run_kernel1(deviceFilter, FILTER_DIMENSION, deviceIn, deviceOut,
                  width, height);
      cudaMemcpy(outputs[1].matrix,deviceOut,nPixel*sizeof(int32_t), cudaMemcpyDeviceToHost);
      run_kernel2(deviceFilter, FILTER_DIMENSION, deviceIn, deviceOut,
                  width, height);
      cudaMemcpy(outputs[2].matrix,deviceOut,nPixel*sizeof(int32_t), cudaMemcpyDeviceToHost);
      run_kernel3(deviceFilter, FILTER_DIMENSION, deviceIn, deviceOut,
                  width, height);
      cudaMemcpy(outputs[3].matrix,deviceOut,nPixel*sizeof(int32_t), cudaMemcpyDeviceToHost);
      run_kernel4(deviceFilter, FILTER_DIMENSION, deviceIn, deviceOut,
                  width, height);
      cudaMemcpy(outputs[4].matrix,deviceOut,nPixel*sizeof(int32_t), cudaMemcpyDeviceToHost);
      run_kernel5(FILTER_DIMENSION, deviceIn, deviceOut,
                  width, height);
      cudaMemcpy(outputs[5].matrix,deviceOut,nPixel*sizeof(int32_t), cudaMemcpyDeviceToHost);
      cudaFree(deviceIn);
      cudaFree(deviceOut);
      cudaFree(deviceFilter);
